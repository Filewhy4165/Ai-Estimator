...
def summarize_payload(payload: dict[str, Any]) -> list[str]:
    ...
    if isinstance(result, dict):
        ...
        if isinstance(line_items, list) and line_items:
            lines.append(f"- Reviewed takeoff line items: {len(line_items)}")
            totals_by_unit = _line_item_totals_by_unit(line_items)
            if totals_by_unit:
                totals = ", ".join(
                    f"{value:g} {unit}" for unit, value in sorted(totals_by_unit.items())
                )
                lines.append(f"- Reviewed takeoff line totals: {totals}")
...
def _line_item_totals_by_unit(line_items: list[object]) -> dict[str, float]:
    ...
    for item in line_items:
        ...
        if isinstance(quantity, str):
            try:
                totals[unit] = totals.get(unit, 0.0) + float(quantity.strip())
            except ValueError:
                continue
    return {unit: round(value, 4) for unit, value in totals.items()}
