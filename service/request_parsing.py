...
def parse_sheet_overrides_json(raw: str | None) -> list[dict[str, Any]] | None:
    ...
    if not isinstance(loaded, list):
        raise ValueError("sheet_overrides_json must be a JSON array.")

    normalized: list[dict[str, Any]] = []
    for index, item in enumerate(loaded):
        if not isinstance(item, dict):
            raise ValueError(f"sheet_overrides_json[{index}] must be an object.")
        ...
        if sheet_id and not _is_reasonable_override_sheet_id(sheet_id):
            if not title:
                ...
                raise ValueError(
                    f"sheet_overrides_json[{index}].sheet_id is invalid. "
        ...
        if source_page_index is None and "source_page_index" in item:
            raise ValueError(
                f"sheet_overrides_json[{index}].source_page_index must be a positive integer."
            )
